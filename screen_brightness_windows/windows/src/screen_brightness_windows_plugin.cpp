...
ScreenBrightnessWindowsPlugin::ScreenBrightnessWindowsPlugin(
  ...
  try {
    GetScreenBrightness(minimum_screen_brightness_, system_screen_brightness_,
                        maximum_screen_brightness_);
...
void ScreenBrightnessWindowsPlugin::RegisterWithRegistrar(
  ...
  plugin->system_screen_brightness_changed_stream_handler_ =
      new ScreenBrightnessChangedStreamHandler();
  std::unique_ptr<flutter::StreamHandler<flutter::EncodableValue>>
      system_screen_brightness_changed_stream_handler_unique_pointer{
          static_cast<flutter::StreamHandler<flutter::EncodableValue> *>(
              plugin->system_screen_brightness_changed_stream_handler_)};
  system_screen_brightness_changed_event_channel->SetStreamHandler(std::move(
      system_screen_brightness_changed_stream_handler_unique_pointer));
  ...
  plugin->application_screen_brightness_changed_stream_handler_ =
      new ScreenBrightnessChangedStreamHandler();
  std::unique_ptr<flutter::StreamHandler<flutter::EncodableValue>>
      application_screen_brightness_changed_stream_handler_unique_pointer{
          static_cast<flutter::StreamHandler<flutter::EncodableValue> *>(
              plugin->application_screen_brightness_changed_stream_handler_)};
  application_screen_brightness_changed_event_channel->SetStreamHandler(
      std::move(
          application_screen_brightness_changed_stream_handler_unique_pointer));
...
void ScreenBrightnessWindowsPlugin::HandleMethodCall(
  ...
  if (method_call.method_name() == "setApplicationScreenBrightness") {
    HandleSetApplicationScreenBrightnessMethodCall(method_call,
                                                   std::move(result));
    return;
  }
...
void ScreenBrightnessWindowsPlugin::HandleGetSystemScreenBrightnessMethodCall(
...
void ScreenBrightnessWindowsPlugin::HandleSetSystemScreenBrightnessMethodCall(
  ...
  const double brightness =
      std::get<double>(args.at(flutter::EncodableValue("brightness")));
  if (std::isnan(brightness)) {
    result->Error("-2", "Unexpected error on null brightness");
    return;
  }

  const long brightness_value =
      GetScreenBrightnessValueByPercentage(brightness);
  try {
    ...
    result->Error("-1", "Unable to change system screen brightness",
                  exception.what());
  }
}

void ScreenBrightnessWindowsPlugin::HandleSystemScreenBrightnessChanged(
  ...
  try {
    const double brightness_percentage =
        GetScreenBrightnessPercentage(brightness);
    system_screen_brightness_changed_stream_handler_
        ->AddScreenBrightnessToEventSink(brightness_percentage);
  } catch (const std::exception &exception) {
    std::cout << exception.what() << std::endl;
  }
}
...
void ScreenBrightnessWindowsPlugin::
    HandleGetApplicationScreenBrightnessMethodCall(
        const std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>>
            result) {
  if (window_handler_ == nullptr) {
    result->Error("-10", "Unexpected error on window handler");
  ...
  try {
    ...
    GetScreenBrightness(minimum_screen_brightness_,
                        application_screen_brightness,
                        maximum_screen_brightness_);
    result->Success(
        GetScreenBrightnessPercentage(application_screen_brightness));
  } catch (const std::exception &exception) {
    result->Error("-11", "Could not found application screen brightness",
                  exception.what());
  }
}
...
void ScreenBrightnessWindowsPlugin::
  ...
  try {
    ...
    result->Error("-1", "Unable to change application screen brightness",
                  exception.what());
  }
}
...
void ScreenBrightnessWindowsPlugin::HandleApplicationScreenBrightnessChanged(
    const long brightness) {
  ...
  try {
    const double brightness_percentage =
        GetScreenBrightnessPercentage(brightness);
    application_screen_brightness_changed_stream_handler_
        ->AddScreenBrightnessToEventSink(brightness_percentage);
  } catch (const std::exception &exception) {
    std::cout << exception.what() << std::endl;
  }
...
void ScreenBrightnessWindowsPlugin::
    HandleHasApplicationScreenBrightnessChangedMethodCall(
        const std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>>
            result) const {
  result->Success(application_screen_brightness_ != -1);
}

void ScreenBrightnessWindowsPlugin::HandleIsAutoResetMethodCall(
    const std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>>
        result) {
  result->Success(is_auto_reset_);
}

void ScreenBrightnessWindowsPlugin::HandleSetAutoResetMethodCall(
    const flutter::MethodCall<flutter::EncodableValue> &call,
    const std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>>
        result) {
  const flutter::EncodableMap &args =
  ...
  const bool is_auto_reset =
      std::get<bool>(args.at(flutter::EncodableValue("isAutoReset")));

  is_auto_reset_ = is_auto_reset;
  result->Success(nullptr);
}

void ScreenBrightnessWindowsPlugin::HandleIsAnimateMethodCall(
    const std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>>
        result) {
  result->Success(is_animate_);
}

void ScreenBrightnessWindowsPlugin::HandleSetAnimateMethodCall(
    const flutter::MethodCall<flutter::EncodableValue> &call,
    const std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>>
        result) {
  const flutter::EncodableMap &args =
      std::get<flutter::EncodableMap>(*call.arguments());
  const bool is_animate =
      std::get<bool>(args.at(flutter::EncodableValue("isAnimate")));

  is_animate_ = is_animate;
  result->Success(nullptr);
}

void ScreenBrightnessWindowsPlugin::HandleCanChangeSystemBrightnessMethodCall(
    const std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>>
        result) {
  result->Success(true);
}
...
ScreenBrightnessWindowsPlugin::HandleWindowProc(HWND hWnd, UINT message,
                                                WPARAM wParam, LPARAM lParam) {
  // allow another plugin to process message
  return std::nullopt;
}

void ScreenBrightnessWindowsPlugin::GetScreenBrightness(
    long &minimum_screen_brightness, long &screen_brightness,
    long &maximum_screen_brightness) {
  DWORD physical_monitor_array_size = 0;
  HMONITOR monitor_handler =
      MonitorFromWindow(window_handler_, MONITOR_DEFAULTTOPRIMARY);
  DWORD minimum_brightness_ = 0, brightness_ = 0, maximum_brightness_ = 0;

  if (!GetNumberOfPhysicalMonitorsFromHMONITOR(monitor_handler,
                                               &physical_monitor_array_size)) {
    throw std::exception("Problem getting numbers of monitor");
  }

  LPPHYSICAL_MONITOR physical_monitor = (LPPHYSICAL_MONITOR)malloc(
      physical_monitor_array_size * sizeof(PHYSICAL_MONITOR));
  ...
  if (!GetPhysicalMonitorsFromHMONITOR(
          monitor_handler, physical_monitor_array_size, physical_monitor)) {
    throw std::exception("Problem getting physical monitors");
  }

  if (!GetMonitorBrightness(physical_monitor->hPhysicalMonitor,
                            &minimum_brightness_, &brightness_,
                            &maximum_brightness_)) {
...
void ScreenBrightnessWindowsPlugin::SetScreenBrightness(
    const long screen_brightness) {}

double ScreenBrightnessWindowsPlugin::GetScreenBrightnessPercentage(
    const long screen_brightness) const {
  if (screen_brightness < 0) {
    return 0;
  }

  return static_cast<double>(screen_brightness - minimum_screen_brightness_) /
         (maximum_screen_brightness_ - minimum_screen_brightness_);
}

long ScreenBrightnessWindowsPlugin::GetScreenBrightnessValueByPercentage(
    const double percentage) const {
  return static_cast<long>(
      (percentage * (maximum_screen_brightness_ - minimum_screen_brightness_)) +
...
void ScreenBrightnessWindowsPluginRegisterWithRegistrar(
